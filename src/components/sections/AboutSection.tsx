  ...
  return (
    <section ref={containerRef} className="relative bg-parchment py-28 px-6 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-4xl">
        <div className="about-head mb-12 text-center">
          ...
          <p className="mt-5 mx-auto max-w-2xl text-base leading-relaxed text-gray-600">
            {t('text')}
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {INFO_ITEMS.map(({ key, valueKey, Icon, accent }) => (
            <div key={key} className="about-card rounded-2xl bg-white p-6 shadow-depth text-center">
              <div
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-gray-400 data-mono">{t(key)}</p>
              <p className="mt-1 font-[var(--font-space-grotesk)] text-lg font-bold text-charcoal">{t(valueKey)}</p>
            </div>
          ))}
        </div>
      </div>
